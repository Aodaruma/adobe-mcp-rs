...
Windows installer を使う場合は、MSI の Custom Setup 画面で After Effects / Premiere Pro / Photoshop / Illustrator / InDesign の bridge component を選択できます。InDesign Startup Scriptは検出済みの現在ユーザーprofileにのみ配置され、profileを検出できない場合は同梱の`install-indesign-bridge.ps1`へ確認済みの`-Destination`を指定します。MSI 本体のファイル配置後、選択された host integration と Codex config の更新は非表示の custom action として実行されるため、通常は別の PowerShell ウィンドウは表示されません。Codex設定がなければ現在ユーザー用に作成し、同名の`mcp_servers.<host>`が既にあればそのtableは変更せず、不足しているhostだけを追加します。

macOS pkgはAEのruntime/startup/shutdownを各After Effectsへ、InDesign Startup Scriptを各`/Applications/Adobe InDesign YYYY/Scripts/Startup Scripts`へ配置します。さらに有効なconsole userのCodex設定へ不足しているMCP serverだけを追加します。console userを安全に特定できない場合はrootの設定を作成せず、MCP設定更新をskipします。
...
Windows MSI は初回インストール時に autostart を勝手に有効化しません。InDesignを含め、ユーザーが既に登録済みの場合だけ upgrade 時に新しいインストール先へ Run key を修復し、通常 uninstall 時にはアンインストールを実行した現在ユーザーの daemon を停止して登録を削除します。別ユーザーの HKCU 登録は各ユーザーで `autostart stop` と `autostart uninstall` を実行してください。
