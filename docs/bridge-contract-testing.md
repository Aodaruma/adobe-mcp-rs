...
テストは `mcp_core::HOST_SPECS` を走査するため、After Effects / Premiere Pro / Photoshop / Illustrator が同じ契約を使います。将来 InDesign の `HostSpec` を追加した場合も、host 固有のテストを複製せず同じ matrix に入ります。
...
2. host を先に起動し、panel/extension の auto-run を有効にしてから daemon を起動する。
...
7. Auto-run を一時停止して短い timeout の request を発行し、Auto-run 復帰後に `get-jsx-result` / `get-results` で late result を回収する。
