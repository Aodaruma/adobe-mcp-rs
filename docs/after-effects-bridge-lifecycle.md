...
repository実装ではAfter Effects bridgeを `Scripts/Startup/mcp-bridge-startup.jsx` からheadlessに開始する。ScriptUI panelや `Auto-run commands` checkboxに依存しない設計だが、対応AE versionでのcold-startは実機未検証。
