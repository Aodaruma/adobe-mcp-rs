...
次の表は Adobe host 自体の能力と、この repository で採用する実行方法を合わせて示します。「実装済み」は repository 上の bridge を指し、「計画」は Issue #21 / #22 で検証する設計を指します。
...
| InDesign | UXP Startup Script `.idjs` を第一候補、UXP plugin を比較候補 | **直接 eval は不可**。UXP Script の固定 permission では `allowCodeGenerationFromStrings = false` | **計画**。受信 source を監査済み一時 `.idjs` にし、`app.doScript(..., UXPSCRIPT, args, undoMode)` で実行する PoC が必要 | **可の見込み**。`doScript` の `withArguments` / `script.args` を使う |

InDesign だけは「raw-first = inline `eval`」ではありません。公式の UXP Script permission では文字列からの code generation が無効なため、任意 source を一時 script file として実行する経路を検証します。これは raw code の柔軟性を維持しつつ、source hash と実行 file を監査しやすい方式でもあります。
...
| InDesign | document / page / story / text / style / link 等を取得可能 | DOM で組版、style、page item、document を更新可能 | PDF / EPUB / image / package 等を host API から実行可能 | `id-mcp` と bridge は未実装。Startup Script の常駐方法を PoC する |
...
|---|---|---|---|---|
| After Effects | 現在は ScriptUI panel の手動 open / Auto-run。Startup JSX / CEP bootstrap を Issue #22 で比較 | **実装済み**。heartbeat、instance routing、instance別 FIFO | inline の統一上限は未実装。file は既定 1 MiB | daemon wait timeout と retained result は実装済み。timeout は実行停止を意味せず、実行中 JSX の強制停止は保証しない |
...
| InDesign | UXP `Startup Scripts` で自動起動し、未解決 Promise / event loop で bridge lifespan を保つ案を Issue #21 で検証 | **計画**。`HostSpec`、heartbeat、instance別 FIFO を追加 | inline ではなく一時 `.idjs`。同じ source / input 上限を適用 | **計画**。daemon timeout と retained result を共通化。`doScript` 実行中の強制停止は前提にしない |
...
`script.execute.inline` の `inline` は「MCP request が source を直接持つ」という transport 上の意味です。InDesign のように host 内では一時 file へ materialize して実行する runtime も、この capability を実装できます。
...
- 4 host の schema / result envelope を揃える。
...
### Phase 2 — InDesign と AE lifecycle を並行 PoC

- **Issue #21:** `id-mcp`、`HostSpec`、UXP Startup Script、`.idjs` materialization + `app.doScript`、structured args / result、heartbeat を検証する。
- **Issue #22:** AE の Startup JSX / CEP bootstrap / 必要時 AEGP を比較し、自動起動と daemon 再接続を検証する。
- 両 PoC から得た lifecycle、permission、timeout の差を capability matrix へ戻す。
