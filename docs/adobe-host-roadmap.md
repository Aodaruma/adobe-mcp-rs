...
この adapter は `mcp-core::HOST_SPECS` と `AppConfig::load_for_host` として実装済みです。schema と移行仕様の詳細は [Bridge protocol v1](bridge-protocol.md) を参照してください。
...
request record は共通の `hostInstance` を使用します。旧 `aeInstance` と旧 heartbeat は Rust 側で互換読み取りし、新規出力は共通 schema に正規化します。
...
- `crates/ps-core` と `crates/ps-mcp` を追加済み
- `src/photoshop/uxp/mcp-bridge-photoshop` を追加済み
- `list-photoshop-instances` を追加済み
- 既存互換の `run-jsx` / `run-jsx-file` を追加済み
- `get-jsx-result` / `get-results` を追加済み
- `get-help` を追加済み
- `run-script` allowlist: `ping`, `getAppInfo`, `listDocuments`, `getActiveDocument`, `listLayers` を追加済み
...
- result schema は共通の `hostInstance` へ移行済み。旧 `aeInstance` の互換読み取りを維持する
