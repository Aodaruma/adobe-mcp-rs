(function () {
  ...
  var instanceIdEl = document.getElementById("instanceId");
  ...
  var heartbeatFileEl = document.getElementById("heartbeatFile");
  ...
  function showFatal(message) {
    ...
    if (!logEl) {
      document.body.innerHTML =
        ...
        '<div style="font-weight:600;margin-bottom:8px;">Illustrator MCP Bridge</div>' +
  ...
  function pollOnce() {
    ...
    if (autoRun) {
      evalScript("aiMcpBridgeCheck()", callback);
    } else {
      evalScript("aiMcpBridgeGetState()", callback);
    }
  }
  ...
  if (autoRunCheckbox) {
    autoRunCheckbox.addEventListener("change", function () {
      ...
      evalScript("aiMcpBridgeSetAutoRun(" + (autoRun ? "true" : "false") + ")", updateState);
    });
  }

  function init() {
    if (!statusEl || !instanceIdEl || !bridgeRootEl || !commandFileEl || !resultFileEl || !heartbeatFileEl || !logEl) {
    ...
    forcePaint();
    evalScript("aiMcpBridgeGetState()", updateState);
    if (!pollTimer) {
