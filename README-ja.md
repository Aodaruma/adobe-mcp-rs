...
Illustrator は `~/Documents/ai-mcp-bridge` 以下で同じ file bridge パターンを使います。CEP panel 上の ExtendScript で任意 JSX/ExtendScript と document/artboard/layer 読み取り用の allowlist script を提供します。Windows installer は、After Effects ScriptUI panel、Premiere / Photoshop UXP panel、Illustrator CEP panel、Codex MCP config を事前選択画面から配置できます。
...
5. Illustrator CEP bridge は export coverage、現行バージョンでの runtime 検証を強化する。UXP は公開 host support が明確になるまで optional 扱いにする。
