...
この repository は、After Effects 専用の `after-effects-mcp-rs` から、Adobe アプリ横断の `adobe-mcp-rs` へ育てる前提でリネームされています。現時点では After Effects 実装が最も進んでおり、Premiere Pro と Illustrator は実験的な実装が入っています。Photoshop は今後追加する段階です。
...
| Illustrator | `ai-mcp` | CEP / ExtendScript panel | 実験的。小さな file bridge surface は実装済み |
...
Illustrator は `~/Documents/ai-mcp-bridge` 以下で同じ file bridge パターンを使います。CEP panel が `instances/` heartbeat と `registry/` 結果保持に対応しますが、インストーラ自動配置はまだ未対応です。
...
5. Illustrator は現行バージョンで CEP bridge を検証し、インストールと export 対応を固める。UXP は third-party host support が明確になるまで optional として扱う。
