...
| Photoshop | `ps-mcp` | UXP panel | 実験的。小さな汎用実行/API読み取り surface |
| Illustrator | `ai-mcp` | CEP / ExtendScript panel | 実験的。小さな汎用実行/document/artboard/layer script surface |
...
Photoshop は `~/Documents/ps-mcp-bridge` 以下で同じ file bridge パターンを使います。UXP bridge は任意 UXP code 実行と、document/layer 読み取り用の小さな allowlist script を提供します。`ps-mcp serve-daemon` は現時点では Premiere と同じ heartbeat daemon 扱いで、After Effects の broker と同等ではありません。

Illustrator は `~/Documents/ai-mcp-bridge` 以下で同じ file bridge パターンを使います。CEP panel 上の ExtendScript で任意 JSX/ExtendScript と document/artboard/layer 読み取り用の allowlist script を提供します。installer 自動配置はまだ未対応です。
...
4. Photoshop UXP bridge は書き込み操作、modal execution policy、installer 対応を強化する。
5. Illustrator CEP bridge は installer 対応、export coverage、現行バージョンでの runtime 検証を強化する。UXP は公開 host support が明確になるまで optional 扱いにする。
