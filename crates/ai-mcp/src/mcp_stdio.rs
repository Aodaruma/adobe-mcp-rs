...
use mcp_core::{
    capabilities_value, prepare_script, validate_and_read_script_file, AppConfig, ILLUSTRATOR_HOST,
};
...
fn run_script_tool(cfg: &AppConfig, bridge: &BridgeClient, args: Value) -> Result<Value> {
    ...
    run_bridge_command(
        ...
        timeout_ms,
        None,
        "Error running Illustrator script",
    )
}
...
fn run_jsx_tool(
    ...
    let prepared = prepare_script(cfg, ILLUSTRATOR_HOST, &args, code, "unsafe", None, limit)?;
    if prepared.preflight_only {
        return tool_json(prepared.preflight_envelope(ILLUSTRATOR_HOST));
    }
...
fn run_jsx_file_tool(cfg: &AppConfig, bridge: &BridgeClient, args: Value) -> Result<Value> {
    ...
    run_bridge_command(
        ...
        timeout_ms,
        Some(audit),
        "Error running Illustrator JSX file",
    )
}
...
fn get_capabilities_tool(cfg: &AppConfig) -> Result<Value> {
    ...
    tool_json(capabilities_value(cfg, ILLUSTRATOR_HOST, instances))
}
...
fn validate_unsafe_mode(args: &Value) -> Result<()> {
    let mode = required_non_empty_string(args, "mode")?;
    if mode != "unsafe" {
        anyhow::bail!("only mode='unsafe' is currently supported for Illustrator JSX execution");
    }
    Ok(())
}
...
mod tests {
    ...
    fn tools_list_contains_generic_execution_tools() {
        ...
        assert!(tools
            .iter()
            .all(|t| t.get("name").and_then(Value::as_str) != Some("list-documents")));
    }
