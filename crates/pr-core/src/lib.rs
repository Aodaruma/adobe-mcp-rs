use mcp_core::{
    merge_canonical_script_tools, PromptArgument, PromptSpec, ToolSpec, PREMIERE_PRO_HOST,
};
...
pub fn tool_specs() -> Vec<ToolSpec> {
    let mut tools = vec![
        ...
        ToolSpec {
            name: "run-jsx-file",
            description: "Run a validated local JavaScript/JSX-style file in Premiere Pro UXP and wait for a result",
            input_schema: json!({
                "type": "object",
                "properties": {
    ...
    merge_canonical_script_tools(&mut tools, PREMIERE_PRO_HOST);
    tools
}
