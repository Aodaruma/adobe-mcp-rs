...
use mcp_core::{AppConfig, PREMIERE_PRO_HOST};
use std::fs;
...
async fn main() -> Result<()> {
    ...
    let cfg = AppConfig::load_for_host(cli.config.as_deref(), PREMIERE_PRO_HOST)?;
...
async fn serve_daemon(once: bool) -> Result<()> {
    ...
    let _pid_file =
        DaemonPidFile::create(PREMIERE_PRO_HOST.bridge_paths().root_dir.join("daemon.pid"))?;
    loop {
